...
namespace casock {
  namespace base {
    Handler::Handler (const FileDescriptor* const pFileDescriptor) : mpFileDescriptor (pFileDescriptor)
    {
...
//      fcntl (fd, F_SETSIG, SIGURG);
      ...
      Dispatcher::getInstance ()->registerHandler (fd, this);
    }

    void Handler::unregister ()
    {
      Dispatcher::getInstance ()->unregisterHandler (mpFileDescriptor->fd ());
    }
  }
}

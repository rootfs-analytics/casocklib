...
#include "casock/rpc/protobuf/server/RPCAcceptorHandler.h"
...
namespace casock {
  namespace rpc {
    namespace protobuf {
      namespace server {
        RPCServerProxy::RPCServerProxy (const uint32& port, google::protobuf::Service* pService) : mpService (pService)
        {
          LOGMSG (HIGH_LEVEL, "RPCServerProxy::RPCServerProxy (const uint32&) - port [%u]\n", port);

          mpServerSocket = new ServerSocket (port);
          mpAcceptorHandler = NULL;
          mpCallQueue = new RPCCallQueue ();
          mpCallHandler = new RPCCallHandler (*mpCallQueue, mpService);
        ...
        {
          mpServerSocket->listen ();
          mpCallHandler->start ();

          if (! mpAcceptorHandler)
            mpAcceptorHandler = new RPCAcceptorHandler (mpServerSocket, *mpCallQueue);

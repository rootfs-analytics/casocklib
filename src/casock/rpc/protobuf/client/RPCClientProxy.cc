...
namespace casock {
  namespace rpc {
    namespace protobuf {
      namespace client {
        RPCClientProxy::RPCClientProxy (const string& host, const unsigned int& port)
        {
          ...
          mpClientSocket = new ClientSocket (host, port);
          mpClientSocket->connect ();
          mpReaderHandler = new RPCReaderHandler (mpClientSocket, this, mCallHash, *mpCallQueue);
